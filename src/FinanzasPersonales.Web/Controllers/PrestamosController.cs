...
public class PrestamosController : BaseController
{
    ...
    public IActionResult Index(int? personaId, string? estado, DateTime? desde, DateTime? hasta)
    {
        ...
        var pagosPorPrestamo = con.Query<PrestamoPago>(
                @"SELECT id, usuario_id AS UsuarioId, prestamo_id AS PrestamoId, fecha, tipo, monto, notas
                  FROM prestamo_pagos WHERE usuario_id=@UsuarioId", new { UsuarioId })
    ...
    // Tablero administrativo de la cartera de prestamos.
    // El filtro de persona aplica a todo; el rango de fechas aplica a la
    // grafica y a los totales de cobros del periodo.
    public IActionResult Tablero(int? personaId, DateTime? desde, DateTime? hasta)
    {
        ...
        var pagos = con.Query<PrestamoPago>(
                @"SELECT id, usuario_id AS UsuarioId, prestamo_id AS PrestamoId, fecha, tipo, monto, notas
                  FROM prestamo_pagos WHERE usuario_id=@UsuarioId", new { UsuarioId })
            .Where(x => idsIncluidos.Contains(x.PrestamoId)).ToList();
        var pagosPorPrestamo = pagos.ToLookup(x => x.PrestamoId);
        foreach (var p in vm.Prestamos)
            CalculoPrestamos.CompletarCalculos(p, pagosPorPrestamo[p.Id].ToList());

        // Serie de cobros: rango indicado o ultimos 12 meses por defecto.
        ...
        // Saldo de capital por persona (solo prestamos activos) para la torta.
        ...
        int? diaPagoInteres, DateTime? fechaPagoCapital, string? notas)
    {
        ...
        if (id == 0)
        {
            var nuevoId = con.ExecuteScalar<int>(
                @"INSERT INTO prestamos (usuario_id, persona_id, fecha, capital, tasa_mensual, dia_pago_interes, fecha_pago_capital, notas)
                  VALUES (@UsuarioId, @personaId, @fecha, @capital, @tasaMensual, @diaPagoInteres, @fechaPagoCapital, @notas)
                  RETURNING id",
                new { UsuarioId, personaId, fecha, capital, tasaMensual, diaPagoInteres, fechaPagoCapital, notas });
            TempData["Ok"] = "Prestamo registrado.";
            return RedirectToAction("Detalle", new { id = nuevoId });
        }

        con.Execute(
            @"UPDATE prestamos SET persona_id=@personaId, fecha=@fecha, capital=@capital, tasa_mensual=@tasaMensual,
                     dia_pago_interes=@diaPagoInteres, fecha_pago_capital=@fechaPagoCapital, notas=@notas
              WHERE id=@id AND usuario_id=@UsuarioId",
            new { id, UsuarioId, personaId, fecha, capital, tasaMensual, diaPagoInteres, fechaPagoCapital, notas });
    ...
    public IActionResult RegistrarPago(int prestamoId, DateTime fecha, string tipo, decimal monto, string? notas)
    {
        ...
        if (tipo == "abono_capital")
        {
            ...
            if (monto > saldo)
            {
                TempData["Error"] = $"El abono ({monto:C0}) supera el saldo de capital ({saldo:C0}).";
                return RedirectToAction("Detalle", new { id = prestamoId });
            }
        }

        con.Execute(
            @"INSERT INTO prestamo_pagos (usuario_id, prestamo_id, fecha, tipo, monto, notas)
              VALUES (@UsuarioId, @prestamoId, @fecha, @tipo, @monto, @notas)",
            new { UsuarioId, prestamoId, fecha, tipo, monto, notas });
    ...
    private static (DateTime? Desde, DateTime? Hasta) NormalizarRango(DateTime? desde, DateTime? hasta)
    {
        desde = desde?.Date;
        hasta = hasta?.Date;
        return desde.HasValue && hasta.HasValue && hasta < desde
            ? (hasta, desde)
            : (desde, hasta);
    }
    ...
    // El prestamo queda 'pagado' cuando los abonos cubren todo el capital.
